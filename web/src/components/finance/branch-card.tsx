...
export function BranchCard({ branch, user, initialGoals, syncId }: BranchCardProps) {
  ...
  const chartGoals: ChartGoalMarker[] = useMemo(() => {
    return metrics.goals.map((g) => {
      ...
      } else if (g.name.toLowerCase().includes("retirement") && user.birthDate) {
        year = new Date(user.birthDate).getFullYear() + 65;
      }

      // Find net worth at that year in simulation snapshots
  ...
  return (
    <Link to="/branches/$name" params={{ name: branch.name }} className="block transition-transform hover:scale-[1.01] active:scale-[0.99]">
      <Card className="flex flex-col md:flex-row overflow-hidden bg-surface-container border-none shadow-none p-6 gap-6 min-h-[280px]">
        ...
        <div className="flex-[4] flex flex-col justify-between border-r border-border/10 pr-6">
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              {branch.id === "main" && (
                <Badge variant="secondary" className="bg-primary/20 text-primary border-none text-[10px] font-mono uppercase tracking-widest">
                  Primary
                </Badge>
              )}
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <span className="text-xl font-mono font-bold text-foreground">
                  {currencyFormatter.format(metrics.retirementNetWorth)}
                </span>
              </div>
              <div>
                <span className="text-[10px] font-bold text-muted-foreground uppercase tracking-widest block mb-1">
                ...
                <span className="text-xl font-mono font-bold text-foreground">
                  {currencyFormatter.format(metrics.terminalNetWorth)}
                </span>
              </div>
            </div>
          ...
          <div className="pt-4 mt-auto border-t border-border/5">
            <div className="flex items-center gap-2">
              ...
              {metrics.isCashflowHealthy ? (
                <Badge variant="outline" className="border-chart-2 text-chart-2 bg-chart-2/10 text-[10px] font-mono uppercase">
                  Healthy
                </Badge>
              ) : (
                <Badge variant="outline" className="border-destructive text-destructive bg-destructive/10 text-[10px] font-mono uppercase flex items-center gap-1">
        ...
        {/* Right Column: Mini Chart (60%) */}
        <div className="flex-[6] min-h-[200px]">
          <TrajectoryChart 
            data={metrics.snapshots.map(s => ({
              date: `${s.year}-01-01`,
              netWorth: s.netWorth,
              equity: s.netWorth, // Simplified for mini chart
              untiedDebt: 0,
            }))} 
            goals={chartGoals} 
            isMini 
