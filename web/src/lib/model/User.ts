...
import {
  applyCommitmentGoalChange,
  type CommitmentGoal,
  type CommitmentGoalChange,
} from "@/lib/model/Goal.Commitment";
import {
  applyMeasurableGoalChange,
  type MeasurableGoal,
  type MeasurableGoalChange,
} from "@/lib/model/Goal.Measurable";
import {
  applyTimeFixedGoalChange,
  type TimeFixedGoal,
  type TimeFixedGoalChange,
...
export function getUserGoals(
  user: User,
  branch: Branch,
  initialGoals: Goal[] = user.initialGoals,
): Goal[] {
  return initialGoals.map((goal) => {
    ...
    switch (goal.type) {
      case GoalType.Commitment:
        return applyCommitmentGoalChange(
          goal as CommitmentGoal,
          change as CommitmentGoalChange,
        );
      case GoalType.Measurable:
        return applyMeasurableGoalChange(
          goal as MeasurableGoal,
          change as MeasurableGoalChange,
        );
      case GoalType.TimeFixed:
        return applyTimeFixedGoalChange(
          goal as TimeFixedGoal,
          change as TimeFixedGoalChange,
        );
      default:
        return goal;
    }
