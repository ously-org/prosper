...
import type { FinancialGroup } from "@/features/FinancialTable/adapters";
    ...
    <>
      ...
      >
        ...
        <TableCell className="px-6 py-4 text-right border-0">
          {group.weightedRate !== undefined && (
            <span
              className={`text-xs font-mono font-bold ${group.weightedRate > 0 ? "text-chart-2" : group.weightedRate < 0 ? "text-destructive" : "text-muted-foreground"}`}
            >
              {group.weightedRate > 0 ? "+" : ""}
          ...
          <div className="flex items-center justify-end gap-2">
            ...
            <div className="w-16">
              <Progress
                ...
                style={
                  {
                    "--progress-foreground": "var(--primary)",
      ...
      {isExpanded &&
        group.items.map((item) => (
          <TableRow
            key={item.id}
            className="bg-surface-container/50 border-b border-border/10 last:border-0 group/item"
          >
            <TableCell className="px-6 py-3 pl-12 flex items-center gap-3 border-0 w-[360px] min-w-[360px] max-w-[360px]">
              ...
              <div className="flex items-center gap-2 flex-1 min-w-0">
                <EditableCell
                  ...
                  onSave={(v) =>
                    stageUpdate(entityType, item.id, { nameTo: v })
                  }
                />
                <RiDeleteBinLine
            ...
            <TableCell className="px-6 py-3 font-mono text-sm text-muted-foreground border-0">
              <EditableCell
                ...
                prefix="$"
                onSave={(v) =>
                  stageUpdate(entityType, item.id, {
                    [valueKey]: (v as number) - item.primaryValue,
                  })
                }
              />
            </TableCell>
            <TableCell className="px-6 py-3 text-right border-0">
              {item.rate !== undefined && (
                <span
                  className={`text-xs font-mono font-medium ${item.rate > 0 ? "text-chart-2" : item.rate < 0 ? "text-destructive" : "text-muted-foreground"}`}
                >
                  {item.rate > 0 ? "+" : ""}
                  {item.rate.toFixed(2)}%
                </span>
